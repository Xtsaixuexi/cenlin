# 🔥 Fireboy and Watergirl 网络版 💧

一个基于 C# TCP 网络编程的双人合作游戏，使用 TcpClient 和 TcpListener 实现网络通信。

![Game Preview](https://img.shields.io/badge/Platform-Windows-blue) ![.NET](https://img.shields.io/badge/.NET-8.0-purple) ![License](https://img.shields.io/badge/License-MIT-green)

## 🎮 游戏简介

Fireboy and Watergirl（森林冰火人）是一款经典的双人合作游戏：
- **Watergirl (水女孩)** 💧 - 蓝色角色，可以通过水/冰区域，但怕火
- **Fireboy (火男孩)** 🔥 - 红色角色，可以通过火区域，但怕水/冰
- **目标** - 两人合作收集宝石，到达各自的出口门

## 📁 项目结构

```
cenlin/
├── Shared/                 # 共享类库
│   ├── GameEntities.cs     # 游戏实体（玩家、地图、宝石等）
│   ├── NetworkProtocol.cs  # 网络协议和消息定义
│   ├── GameConfig.cs       # 游戏配置常量
│   ├── LevelGenerator.cs   # 关卡生成器
│   └── Shared.csproj
│
├── Server/                 # 游戏服务器 (TcpListener)
│   ├── Program.cs          # 服务器入口
│   ├── GameServer.cs       # TCP服务器实现
│   ├── GameLogic.cs        # 游戏逻辑处理
│   └── Server.csproj
│
├── GameClient/             # WinForms图形化客户端 (TcpClient)
│   ├── Program.cs          # 客户端入口
│   ├── MainForm.cs         # 主游戏窗口
│   ├── GameRenderer.cs     # 游戏图形渲染器
│   ├── NetworkClient.cs    # TCP客户端实现
│   ├── ConnectDialog.cs    # 连接对话框
│   └── GameClient.csproj
│
├── FireboyAndWatergirl.sln # Visual Studio 解决方案
└── README.md
```

## 🛠️ 环境要求

- **操作系统**: Windows 10/11
- **.NET SDK**: 8.0 或更高版本
- **IDE**: Visual Studio 2022 (可选) 或 VS Code

### 安装 .NET SDK

1. 访问 https://dotnet.microsoft.com/download
2. 下载并安装 .NET 8.0 SDK
3. 验证安装：
```powershell
dotnet --version
```

## 🚀 运行方法

### 方法一：使用命令行

**第一步：启动服务器**
```powershell
cd C:\Users\zhang\Desktop\cenlin\Server
dotnet run
```

**第二步：启动客户端（打开新的终端窗口）**
```powershell
cd C:\Users\zhang\Desktop\cenlin\GameClient
dotnet run
```

**第三步：再启动一个客户端（另一个玩家）**
```powershell
cd C:\Users\zhang\Desktop\cenlin\GameClient
dotnet run
```

### 方法二：使用 Visual Studio

1. 双击打开 `FireboyAndWatergirl.sln`
2. 右键解决方案 → 设置启动项目 → 多个启动项目
3. 将 Server 和 GameClient 都设为"启动"
4. 按 F5 运行

### 方法三：使用批处理文件

1. 双击 `启动服务器.bat` - 启动服务器
2. 双击 `启动客户端.bat` - 启动玩家1
3. 再次双击 `启动客户端.bat` - 启动玩家2

## 🎯 游戏控制

| 按键 | 功能 |
|------|------|
| `A` / `←` | 向左移动 |
| `D` / `→` | 向右移动 |
| `W` / `↑` / `空格` | 跳跃 |
| `R` | 重新开始 |
| `Enter` | 发送聊天消息 |

## 🌐 网络架构

```
┌─────────────────┐         TCP          ┌─────────────────┐
│   GameClient    │◄────────────────────►│                 │
│   (WinForms)    │     TcpClient        │                 │
│   Watergirl     │                      │   GameServer    │
└─────────────────┘                      │  (TcpListener)  │
                                         │                 │
┌─────────────────┐         TCP          │   端口: 9527    │
│   GameClient    │◄────────────────────►│                 │
│   (WinForms)    │     TcpClient        │                 │
│   Fireboy       │                      └─────────────────┘
└─────────────────┘
```

### 网络协议

- 使用自定义二进制协议
- 消息格式: `[4字节长度][类型名长度][类型名][JSON数据长度][JSON数据]`
- 支持的消息类型：
  - `ConnectMessage` - 连接请求
  - `PlayerInputMessage` - 玩家输入
  - `GameStateMessage` - 游戏状态同步
  - `ChatMessagePacket` - 聊天消息

## 🎨 游戏元素

| 符号 | 名称 | 说明 |
|------|------|------|
| `I` | Watergirl | 蓝色玩家角色 |
| `F` | Fireboy | 红色玩家角色 |
| `█` | 墙壁 | 不可通过 |
| `═` | 平台 | 可站立 |
| `~` | 冰/水区域 | 只有Watergirl可通过 |
| `^` | 火区域 | 只有Fireboy可通过 |
| `≈` | 毒水 | 两者都会死亡 |
| `◆` | 蓝宝石 | Watergirl收集 |
| `◇` | 红宝石 | Fireboy收集 |
| `▣` | 蓝门 | Watergirl出口 |
| `▢` | 红门 | Fireboy出口 |

## 📝 开发说明

### 核心类说明

**服务器端 (Server)**
- `GameServer.cs` - 使用 TcpListener 监听连接，管理玩家会话
- `GameLogic.cs` - 游戏物理引擎，碰撞检测，胜负判定

**客户端 (GameClient)**
- `NetworkClient.cs` - 使用 TcpClient 连接服务器
- `GameRenderer.cs` - 使用 GDI+ 绘制游戏画面
- `MainForm.cs` - WinForms 主窗口，处理用户输入

**共享库 (Shared)**
- `NetworkProtocol.cs` - 消息序列化/反序列化
- `GameEntities.cs` - 游戏实体定义

### 扩展建议

1. **添加更多关卡** - 修改 `LevelGenerator.cs`
2. **添加音效** - 使用 NAudio 库
3. **添加更多角色特效** - 修改 `GameRenderer.cs`
4. **支持局域网自动发现** - 添加 UDP 广播

## 📜 许可证

MIT License - 可自由使用和修改

---

**技术栈**: C#, .NET 8.0, WinForms, TCP/IP
